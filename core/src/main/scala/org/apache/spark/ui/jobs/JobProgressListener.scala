 ...
 * Tracks task-level information to be displayed in the UI.
  * 跟踪在UI中显示的task-level信息。
 *
 * All access to the data structures in this class must be synchronized on the
 * class, since the UI thread and the EventBus loop may otherwise be reading and
 * updating the internal data structures concurrently.
  *
  * 该类中所有对数据结构的访问必须在类上同步，因为UI线程和EventBus循环可能同时读取和更新内部数据结构。
  *
  *   JobProgressListener是SparkCOntext中一个重要的组成部分，通过监听listenerBus中的事件更新任务进度。
  *   SparkStatusTracker和SparkUI实际上也是通过JobProgressListener来实现任务状态跟踪的。
  *
  *   JobProgressListener的作用是通过HashMap，ListBuffer等数据结构存储JobId及其对应的JobUIData信息，
  *   并且按照激活，完成，失败等Job状态统计。对于StageId，StageInfo等信息按照激活，完成，忽略，失败等
  *   Stage状态统计，并且存储StageId与JobId的一对多关系。这些统计信息最终被JobPage和StagePage等页面的
  *   访问和访问。
  *
  *   JobProgressListener实现了onJobStart，onJobEnd,onStageCompleted,onStageSubmitted,onTaskStart,onTaskEnd
  *   等方法，这些方法正是在listenerBus的驱动下，改变JobProgressListener中的各种Job,Stage相关数据。
  *
  *   这个类将会在2.2版本以后移除
 */
@DeveloperApi
...
class JobProgressListener(conf: SparkConf) extends SparkListener with Logging {
  ...
  // sparkUI中默认保留SparkUI.DEFAULT_RETAINED_STAGES=1000个Stages
  ...
  /**
    * 我们可以通过确保跟踪非活动的工作和阶段的集合不受约束地增长，并且在Spark空闲时最终变为空的集合来测试内存泄漏。
    * 让我们把我们的集合划分为一个应该是空的，一个是空闲的，一个应该是硬的或软有限的大小。单元测试使用这些方法，但是
    * 这里定义了这些方法，以便人们在添加新集合时不会忘记更新测试。有些集合有多个层次的嵌套，等等，因此这让我们可以
    * 自定义每个结构的“大小”概念:
    * */


  // These collections should all be empty once Spark is idle (no active stages / jobs):
  // 这些集合应该是空的，一旦Spark空闲(没有活动的阶段/工作)
  ...
  private def calculateNumberToRemove(dataSize: Int, retainedSize: Int): Int = {
    // 如果stages减去1000等于A,A和100相比较，移除大于100 小于等于 <超出1000的数量
