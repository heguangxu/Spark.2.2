...
private[spark] object UIWorkloadGenerator {
  ...
  def main(args: Array[String]) {
    ...
    if (schedulingMode == SchedulingMode.FAIR) {
       // 这里所说的Application中job的调度指的是单个应用中各个action的执行，
      // 其实就是各个stage的task的运行。Spark的默认调度就是FIFO先进先出，
      // 最早的job的最早的stage先运行，然后运行之后的stage。这样就会造成先提交的
      // 耗时很久的作业在运行的时候阻塞住晚提交的原本运行时间很短的作业，系统
      // 得不到很好的响应。因此Saprk0.8之后提供了一个大致公平的调度算法FAIR，
      // 这种模式最适合多用户设置。
