 ...
 * Utilities for launching a web server using Jetty's HTTP Server class
  *
  * 使用Jetty的httpserver类，启动一个web server的实用工具
 */
private[spark] object JettyUtils extends Logging {
  ...
  /** Create a handler for serving files from a static directory  从静态目录中创建服务文件的处理程序。*/
   ...
   * context handlers.
    *
    * 尝试启动一个Jetty server绑定指定的主机名：端口使用给定的context handlers.
   *
   ...
   * found. Return the jetty Server object, the chosen port, and a mutable collection of handlers.
    *
    * 如果给定的port被占用了，那么就累加这个端口，一直到找到一个端口是空闲的，返回一个jetty Server对象，和一个处理程序的一个可变集合。
    *
   */
