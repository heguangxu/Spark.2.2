...
private[ui] class AllJobsPage(parent: JobsTab) extends WebUIPage("") {
  ...
  /**
    * JobsTable用来生成表格数据，表格中的每行数据又是通过makeRow方法渲染的。
    * @param request
    * @param tableHeaderId
    * @param jobTag
    * @param jobs
  ...
  def render(request: HttpServletRequest): Seq[Node] = {
    ...
    listener.synchronized {
      ...
      val completedJobs = listener.completedJobs.reverse // 获取当前application中完成状态的job
      val failedJobs = listener.failedJobs.reverse // 获取当前application中失败状态的job
