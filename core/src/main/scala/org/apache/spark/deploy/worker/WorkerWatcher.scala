 ...
 * Provides fate sharing between a worker and its associated child processes.
  *
  * 连接到工作进程的端点，如果连接断开，则终止JVM。
  * 提供工作人员与相关的子进程之间的命运共享。
 */
private[spark] class WorkerWatcher(
  ...
  // Used to avoid shutting down JVM during tests    用于避免在测试期间关闭JVM
  ...
  // 在一般情况下，exitNonZero将调用system .exit(-1)关闭JVM。
  // 在单元测试中，用户应该调用“setTesting(true)”因此，exitNonZero将把“isShutDown”设置为true
  // ，而不是调用“system . exit”。用户可以检查“isShutDown”是否被调用“exitNonZero”。
