...
/**
  ...
  *
  * 该套件测试了Spark独立调度程序的容错，主要是主程序。为了更紧密地模拟真正的分布式集群，使用Docker。执行使用
  * ./bin/spark-class org.apache.spark.deploy.FaultToleranceTest
  *
  ...
  *
  * 确保在SPARK_DAEMON_JAVA_OPTS环境包含以下属性:
  *   - spark.deploy.recoveryMode=ZOOKEEPER
  *   - spark.deploy.zookeeper.url=172.17.42.1:2181
  * 注意,172.17.42.1是默认的docker ip主机和2181是默认ZK端口。
  *
  * In case of failure, make sure to kill off prior docker containers before restarting:
  *   docker kill $(docker ps -q)
  *
  * 如果出现故障，请确保在重新启动之前杀死之前的docker容器:
  * docker工人杀死$(docker ps -q)
  *
  ...
  *
  * 不幸的是,由于Docker依赖此套件，在working节点没有安装Docker将不会自动的运行。假设安装了Docker,以下是假定:
  *   Docker可以不使用sudo进行运行(参见http://docs.docker.io/en/latest/use/basics/)
  *   ——Docker images tagged spark-test-master和spark-test-worker建造的docker/ directory， Run 'docker/spark-test/build' to generate these.
  *
  */
