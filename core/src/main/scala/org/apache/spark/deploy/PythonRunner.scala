...
/**
  ...
  *
  * 一个用于启动Python应用程序的主类。它执行python作为子流程，然后将其连接回JVM以访问系统属性等。
  */
object PythonRunner {
  def main(args: Array[String]) {
    ...
    // Format python file paths before adding them to the PYTHONPATH    // 在添加python路径之前，格式化python文件路径
    val formattedPythonFile = formatPath(pythonFile)
    val formattedPyFiles = formatPaths(pyFiles)

    // Launch a Py4J gateway server for the process to connect to; this will let it see our
    // Java system properties and such
    // 为连接的进程启动Py4J网关服务器;这将让它看到我们的Java系统属性等
    ...
    // 等待网关服务器启动，以便我们知道它绑定到哪个端口。在初始化socket之后，“gatewayServer.start() 将启动一个新的线程并在那里运行服务器代码，
    // 因此当服务器准备好服务连接时，启动的线程将会结束。
    thread.join()
  ...
  /**
    ...
    * 格式化python文件路径，以便正确地将其添加到PYTHONPATH中。
    *
  ...
  /**
    ...
    *
    * 格式以逗号分隔列表中的每个python文件路径的路径,所以它可以被添加到PYTHONPATH正确。
    */
