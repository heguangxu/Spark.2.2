...
private[spark] object InternalAccumulator {
  ...
  // InternalAccumulator意思：内部累加器 与内部累加器相关的字段和方法的集合，表示任务级度量。

  // Prefixes used in names of internal task level metrics  内部task级别的监控
  ...
  // Names of internal task level metrics 内部task级别的监控
  ...
  // Names of shuffle read metrics  shuffle读取的度量
  object shuffleRead {
  ...
  // Names of shuffle write metrics   shuffle 写入的度量
  object shuffleWrite {
