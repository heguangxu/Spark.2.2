...
// 待办事项：在SparkContext中暴露一个半私人的jdbcRDD功能函数
/**
  * An RDD that executes a SQL query on a JDBC connection and reads results.
  * For usage example, see test case JdbcRDDSuite.
  *
  * 在JDBC connection连接上执行一个SQL查询，返回一个RDD查询结果，例如，查看测试用例JdbcRDDSuite
  *
  * @param getConnection a function that returns an open Connection.
  *   The RDD takes care of closing the connection.
  *           一个函数，返回一个打开的连接。RDD负责关闭连接。
  ...
  *                 分区的数量。给定一个下界的1、20，那么分区的数量是2，查询将被执行两次，一次（1, 10）和一次（11, 20）
  * @param mapRow a function from a ResultSet to a single row of the desired result type(s).
  *   This should only call getInt, getString, etc; the RDD takes care of calling next.
  *   The default maps a ResultSet to an array of Object.
  *               一个功能从结果集（ResultSet）一个单一的行所需的结果类型（S）。这应该只调用getString，getInt，等；
  *               RDD负责屌用下一个。默认映射一个ResultSet对象数组。
  */
...
object JdbcRDD {
  ...
  /**
    ...
    *
    * 创建一个RDD执行JDBC连接SQL查询和读取结果。使用的例子，看看测试用例javaapisuite.testjavajdbcrdd。
    *
    * @param connectionFactory a factory that returns an open Connection.
    *   The RDD takes care of closing the connection.
    *                     一个JDBC连接工厂，返回打开的连接，RDD负责关闭连接
    * @param sql the text of the query.
    *   The query must contain two ? placeholders for parameters used to partition the results.
    *   For example,
    *   {{{
  ...
  /**
    ...
    *
    * 创建一个RDD执行JDBC连接SQL查询和读取结果。每一行转换成`Object`数组。使用的例子，看看测试用例javaapisuite.testjavajdbcrdd。
    *
    *
    * @param connectionFactory a factory that returns an open Connection.
    *   The RDD takes care of closing the connection.
