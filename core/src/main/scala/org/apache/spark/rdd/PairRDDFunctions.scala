...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
  ...
  /**
    ...
    *
    * 通用的功能相结合的元素为每个关键使用一组自定义聚集函数。把一个RDD[(K, V)] 变为一个类型为RDD[(K, C)]的结果，
    * 一个“组合式”C
    *
    ...
    *
    *  - ` createcombiner `，把v变成c（例如，创建一个元素的列表）
    *  - ` mergevalue `，合并V到C（例如，将它添加到一个列表的末尾）
    *  - ` mergecombiners `，将两个C的联合为一个。
    *
    * In addition, users can control the partitioning of the output RDD, and whether to perform
    * map-side aggregation (if a mapper can produce multiple items with the same key).
    *
    * 此外，用户可以控制输出RDD的分区，和是否执行map-side聚集（如映射可以产生相同的密钥使用相同的key）。
    *
  ...
  def combineByKeyWithClassTag[C](
    ...
    // 由于其ZippedPartitionsRDD2的partitioner为none，而Some（partitioner）为1个HashPartitioner，
    // 因此跳过，除非生成parallelizerdd的时候制定分区函数为HashParititioner,则直接生成MapPartitionsRDD
    ...
    if (self.partitioner == Some(partitioner)) {
      ...
      // 生成1个ShuffledRDD
      //groupRdd本质上是个ShuffledRDD，其继承自RDD，改写了RDD的getDependencies函数，
      // 其依赖关系为ShuffleDependency，分区函数为HashPartitioner，locationPrefs为Nil。
      // 因此最终的Rdd关系如下图：
      //    ParallelCollectionRDD -----\
      //                              ZippedPartitionsRDD   ------>  ShuffledRDD
      //    ParallelCollectionRDD -----/
  ...
  /**
    ...
    *
    * 使用自定义集合函数集为每个键组合元素的泛型函数。这种方法是在向后兼容性。它没有为shuffle提供
    * combiner classtag（合成器标签信息）信息。
    *
  ...
  /**
    ...
    * classtag information to the shuffle.
    *
    * 对combinebykeywithclasstag哈希分区输出RDD的简化版。这种方法是在向后兼容性。它没有为shuffle提供combiner classtag（合成器标签信息）信息。
    *
    * @see `combineByKeyWithClassTag`
    */
  def combineByKey[C](
  ...
  /**
    ...
    * Simplified version of combineByKeyWithClassTag that hash-partitions the output RDD.
    *
    * 对combinebykeywithclasstag哈希分区输出RDD的简化版本。
    *
    */
  @Experimental
  def combineByKeyWithClassTag[C](
  ...
  /**
    ...
    *
    * 汇总每个key的值，使用给定的combine函数和一个中性的"zero value"。这个函数可以返回不同的结果类型，U，值的类型，V
    * 因此，我们需要一个操作合并成一个U和V的一个操作合并两个U的，如在scala.traversableonce。前一个操作用于合并分区内的值，
    * 后者用于合并分区之间的值。为了避免内存分配，这两个函数都可以修改和返回第一个参数，而不是创建一个新的参数。
    *
    */
