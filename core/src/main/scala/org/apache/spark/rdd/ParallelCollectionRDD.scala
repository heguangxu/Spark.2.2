...
private[spark] class ParallelCollectionRDD[T: ClassTag](
  ...
  //TODO:现在，每一个拆分都发送其完整的数据，即使在稍后的RDD链中被缓存。将数据写入DFS中的文件并在拆分中读取它可能是值得的。
  //更新:一个并行的集合可以被检查指向HDFS，它实现了这个目标。
...
private object ParallelCollectionRDD {
   ...
   * is an inclusive Range, we use inclusive range for the last slice.
    *
    *
    * ParallelCollectionRDD的slice方法，用于将一个集合切分成numSlices个子集合，这样做可以让多个Receiver有效地运行在Spark上。
    * 此处时间是哪个是讲Receiver划分为多个序列，然后每个序列创建一个ParallelCollectionParation,ParallelCollectionParation
    * 的参数实际上由numSlices决定。
   */
