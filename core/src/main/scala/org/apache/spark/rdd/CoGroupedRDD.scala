...
/**
  ...
  * task closure.
  *
  * RDD和splitIndex的引用是瞬态的，因为冗余信息存储在CoGroupedRDD对象中。因为CoGroupedRDD是分别从CoGroupPartition序列化，
  * 如果RDD和splitindex不是瞬态的，他们将在task任务结束的时候，引入两次。
  *
  */
...
/**
  ...
  *
  *                  映射父RDD的窄依赖变量，一对一的依赖，narrowDeps有一个NarrowCoGroupSplitDep（分区依赖的描述）
  *                  分区在相应下标。narrowDeps的大小应该等于父narrowDeps的大小。
  *
  */
...
/**
  ...
  *
  * 一个RDD cogroups 它自己的父RDD。每个key都在父RDD中，结果RDD包含一个tuple元组，一个key对应多个value
  *
