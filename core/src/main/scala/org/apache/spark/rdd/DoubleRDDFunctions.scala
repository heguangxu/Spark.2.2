...
class DoubleRDDFunctions(self: RDD[Double]) extends Logging with Serializable {
  ...
  /**
    ...
    *
    * 返回一个[[org.apache.spark.util.StatCounter]]对象意思是捕捉的，在一个运行的RDD的元素变异数。
    */
  ...
  /**
    ...
    *
    * 计算该RDD的元素样本的标准偏差（校正除以n-1而不是在N估计的标准偏差偏差）。
    *
    */
  ...
  /**
    ...
    *
    * 计算该RDD的元素的样本方差（校正除以n-1而不是在N估计方差的偏差）。
    *
    */
  ...
  /**
    ...
    *
    * 计算使用的桶bucketcount数数据均匀分布的最小和最大之间的RDD直方图。例如，如果最小值为0，最大值为100，
    * 并且有两个桶，结果桶将为[ 0, 50 ] [ 50, 100 ]。bucketcount必须至少为1，
    * 如果RDD包含无限，NaN 抛出一个异常
    * 如果在RDD的元素不改变（max == min）总是返回一个单桶。
    *
    */
