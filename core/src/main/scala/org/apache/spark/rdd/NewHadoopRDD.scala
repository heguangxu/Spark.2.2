...
/**
  ...
  *
  * @注意 直接实例化这个类不推荐，请使用`org.apache.spark.SparkContext.newAPIHadoopRDD()`
  *
  */
@DeveloperApi
class NewHadoopRDD[K, V](
  ...
  def getConf: Configuration = {
    ...
    if (shouldCloneJobConf) {
      ...
      // Hadoop的配置对象不是线程安全的，这可能会导致各种各样的问题，如果一个工作修改配置而另读它（spark-2546，spark-10611）。
      // 这个问题很少发生，因为大多数工作将配置视为不可变的。在这里实现的一个解决方案是克隆配置对象。不幸的是，这种克隆可能非常昂贵。
      // 为了避免对不受这些线程安全问题影响的工作负载和Hadoop版本的意外性能回归，默认情况下禁用此克隆。
      ...
      NewHadoopRDD.CONFIGURATION_INSTANTIATION_LOCK.synchronized {
        ...
        // 配置通过实际上是一个jobconf可能包含证书。保持这些凭据正确我们必须创建一个新的jobconf不配置。
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    val iter = new Iterator[(K, V)] {
      ...
      // 为block块信息设置InputFileBlockHolder文件
      ...
      // 找到一个函数，该函数返回该线程读取的文件系统字节，在创建RecordReader之前做这些，因为RecordReader的构造函数可以读取一些字节
      ...
      // 我们输入的字节从线程本地Hadoop文件系统的统计数据。如果我们联合起来，但是，我们有可能计算在相同的任务，
      // 在同一个线程的多个分区，在这种情况下，我们需要避免重写值由以前的分区写入（spark-13071）。
  ...
  override def persist(storageLevel: StorageLevel): this.type = {
    if (storageLevel.deserialized) {
      // 缓存newhadooprdds作为反序列化的对象通常会导致意外的行为，因为Hadoop的RecordReader重用同一写对象的所有记录。
      // 使用地图转换复制记录。
...
private[spark] object NewHadoopRDD {
  /**
    ...
    *
    * 配置的构造函数不是线程安全的（见spark-1097和hadoop-10456）。因此，我们使用同步锁在调用new Configuration()之前。
    *
    */
  ...
  /**
    ...
    *
    * 类似[[org.apache.spark.rdd.MapPartitionsRDD]]，但在一个inputsplit照顾两个特定的功能而不是索引的分区。
    *
    */
