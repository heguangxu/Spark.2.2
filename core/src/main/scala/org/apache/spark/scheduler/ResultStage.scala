 ...
 * of the RDD, for actions like first() and lookup().
  *
  * resultstage在一个RDD分区上应用一个函数来计算action的结果。ResultStage对象捕获execute的函数，
  * “func”，它将应用于每个分区，以及分区id的集合“分区”。某些阶段可能不会在RDD的所有分区上运行，比如first()和lookup()。
 */
private[spark] class ResultStage(
   ...
   * (e.g., because the job was cancelled).
    *
    * 这个result stage的活动的job。如果工作已经完成了，会是空的吗(例如：job被取消了)
   */
