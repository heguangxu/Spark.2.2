 ...
 * A location that includes both a host and an executor id on that host.
  ...
  *
  * ExecutorCacheTaskLocation: 代表 partition 数据已经被 cache 到内存，比如 KafkaRDD 会将 partitions 都 cache 到内存，
  * 其 toString 方法返回的格式如 executor_$host_$executorId
 */
private [spark]
 ...
 * A location on a host that is cached by HDFS.
  ...
  * 代表数据存储在 hdfs 上
  *
  * HDFSCacheTaskLocation：代表 partition 数据存储在 hdfs 上，比如从 hdfs 上加载而来的 HadoopRDD 的 partition，
  * 其 toString 方法返回的格式如 hdfs_cache_$host
 */
