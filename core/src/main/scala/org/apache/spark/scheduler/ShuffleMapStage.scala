...
private[spark] class ShuffleMapStage(
   ...
   * will always be a subset of the partitions that the TaskSetManager thinks are pending).
    *
    * 还没有计算过的分区，或者是在executor已经计算过了，但是丢失了，因此应该重新计算。这个变量被DAGScheduler用来决定
    * 一个stage什么时候完成。在这个阶段的积极尝试中或在早期尝试中，任务成功都可以导致被遗弃的ids从pending分区中移除。
    * 因此，这个变量可能与TaskSetManager中的待处理任务不一致(这里存储的分区将始终是TaskSetManager认为挂起的分区的子集)。
    *
   */
