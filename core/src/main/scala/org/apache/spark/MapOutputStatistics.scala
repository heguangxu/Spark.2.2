...
// 保存映射阶段中输出大小的统计信息。可能在未来成为一个developerapi。
// @param shuffleId ID of the shuffle  被清洗的ID
// @param bytesByPartitionId  近似的输出字节数为每一个地图的输出分配（可能是inexact由于使用compressed地图的statuses）