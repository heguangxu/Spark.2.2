 ...
 * serialized ByteBuffers.
  *
  * 在内存中存储块，要么作为反序列化的Java对象的数组，要么作为序列化的bytebuffer。
  *
...
private[spark] class MemoryStore(
  ...
  // 注意:对内存分配的所有更改，特别是设置块、驱逐块、获取或释放unroll内存，必须在“memoryManager”上同步!

  // entries持有的是内存中currentMemory。
  private val entries = new LinkedHashMap[BlockId, MemoryEntry[_]](32, 0.75f, true)

  // A mapping from taskAttemptId to amount of memory used for unrolling a block (in bytes)
  // All accesses of this map are assumed to have manually synchronized on `memoryManager`
  // 从taskUNK tid映射到用于展开块(以字节为单位)的内存数量，假定在“memoryManager”上手动同步了该映射的所有访问。
  ...
  // Initial memory to request before unrolling any block 在展开任何块之前要请求的初始内存
  private val unrollMemoryThreshold: Long =
    conf.getLong("spark.storage.unrollMemoryThreshold", 1024 * 1024)

  /** Total amount of memory available for storage, in bytes.
    * 存储中可用的内存总数,(当前Driver或者executor的最大内存)
    * */
  ...
  // 20=>17/12/05 11:56:50 INFO MemoryStore: MemoryStore started with capacity 873.0 MB
  logInfo("MemoryStore started with capacity %s".format(Utils.bytesToString(maxMemory)))

  /** Total storage memory used including unroll memory, in bytes.
    * 当前Driver或者executor的已经使用的内存
    * */
   ...
   *
    * 使用size去测试MemoryStore中是否有足够的内存，如果内存足够，创建ByteBuffer并且写入到内存，否则将不会创建ByteBuffer
   ...
   * so we won't acquire more memory than is actually needed to store the block.
    *
    * 迭代器可能太大，无法在内存中实现和存储。为了避免OOM异常，这个方法将逐步打开迭代器，
    * 同时定期检查是否有足够的空闲内存。如果成功地物化了块，那么在物化过程中使用的临时的未滚动内存将被“转移”到存储内存中，
    * 因此我们不会获得比实际需要的存储块更多的内存。
   *
   ...
   *         block.
    *
    *
    *        在成功的情况下，估计存储数据的大小。如果出现故障，返回包含该块值的迭代器。返回的迭代器将由部分展开的块
    *        和原始输入迭代器的其余元素的组合来支持。调用者必须完全使用此迭代器或调用“close()”在它上面，
    *        是为了释放被部分展开的块所消耗的存储内存。
    *
    *
    *  这个方法在1.5以前是unrollSafely
    *
    *     为了防止写入内存的数据过大，导致内存溢出，Spark采用了一种优化方案；在正式写入内存之前，先用逻辑方式申请内存，
    *  如果申请成功，在写入内存，这个过程称为安全展开。下表是putIteratorAsValues方法一些变量及其算法的定义。
    *
    *     变量名                         含义
    *  elsementsUntrolled             展开元素的数量
    *  initialMemoryThreshold         每个线程用来展开Block的初始内存阈值，可以修改属性spark.storage.unrollMemoryThreshold改变大小
    *  memoryCheckPeriod              经过多次展开内存后，判断是否需要申请更多的内存
    *  memoryThreshold                当前线程保留的用于特殊展开操作的内存阈值
    *  memoryGrowthFactor             内存请求因子，与vector的大小乘积，减去memoryThreshold作为新请求的内存大小
    *  previousMemoryReserved         之前当前线程已经展开的主流的内存大小，当前线程增加的展开内存，最后会释放
    *  vector                         跟踪展开内存信息
    ...
    *  currentUnrollMemory            unrollMemoryMap中所有展开的Block的内存之和，既当前Driver或者Executor中所有线程展开的Block的内存之和
    *  unrollMemoryMap                当前Driver或者Executor中所有线程展开的Block都存入此map中，key为线程Id,value为线程展开的所有块的内存的大小总和
    *  currentSize                    vector中跟踪的对象的总大小
    *  keepUnrolling                  标记是否有足够的内存可以持续展开Block  keepUnrolling = freeMemory > currentUnrollMemory + memory(要分配的内存)
    *
    *
    * 下面我们李处展开内存的步骤：
    *   1.申请memoryThreshold的初始大小为initialMemoryThreshold.
    *   2.如果Iterator[Any]中油元素并且keepUnrolling为True,则向vector中添加Iterator[Any]的对象，elementsUnrolled
    *     自增1.如果Iterator[Any]中没有元素或者keepUnrolling不为True，则跳转到第4步。
    *   3.如果elementsUnrolled % memoryCheckperiod == 0,则开始检查currentSize是否已经比memoryThreshold大？假如
    *     currentSize已经超过了memoryThreshold,则需要再申请内存，如果申请内存大小amountToRequest = currentSize * memoryGrowthFactor - memoryThreshold.
    *     如果申请失败，但是maxUnrollMemory > currentUnrollMemory,则要求释放当前Driver或者Executor.释放内存必然伴随
    *     着其他Block被移入硬盘或者彻底删除。这些块的状态会在释放后返回。此时再次申请内存，memoryThreshold增加的大小为amountToRequest,返回第2步。
    *   4.根据是否将block完整的放入内存，以数组或者迭代器形式返回vector的数据。
    *   5.最后在fianlly语句块里，还会计算本次展开块实际占用的空间amountToRelease,并且更新unrollMemoryMap中当前线程
    *     占用的内存大小，并且减去amountToRelease，如果unrollMemoryMap中当前线程占用的内存大小等于0，则从unrollMemoryMap
    *     中完全清除此线程的数据。
   */
  private[storage] def putIteratorAsValues[T](
    ...
    // How often to check whether we need to request more memory  多久检查一次我们是否需要更多的内存
    val memoryCheckPeriod = 16
    // Memory currently reserved by this task for this particular unrolling operation 当前为这个特殊的展开操作而保留的内存
    var memoryThreshold = initialMemoryThreshold
    // Memory to request as a multiple of current vector size 内存请求作为当前矢量大小的倍数
    ...
    if (keepUnrolling) {
      ...
      if (enoughStorageMemory) {
        ...
        //INFO MemoryStore: Block input-0-1517373103200 stored as values in memory
        // (estimated size 1352.0 B, free 872.9 MB)
