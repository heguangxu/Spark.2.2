 ...
 * Stores BlockManager blocks on disk.
  * 在磁盘上存储区块管理器块。
  *
  * 当MemoryStore没有足够空间时，就会使用DiskStore将块存入磁盘。DiskStore继承DiskStore，
  * 并且实现了getBytes,putBytes,putArray,putIterator等方法。
 */
private[spark] class DiskStore(
  ...
  // 该方法作用是通过DiskBlockManager的getFile方法获取文件，然后使用NIO的Channel将ByteBuffer写入文件
  ...
  def getBytes(blockId: BlockId): BlockData = {
    ...
    securityManager.getIOEncryptionKey() match {
      case Some(key) =>
        ...
        // 加密的块不能被内存映射;返回一个进行解密的特殊对象，并提供InputStream / FileRegion实现来读取数据。
      ...
      case _ =>
        ...
        if (blockSize < minMemoryMapBytes) {
          // For small files, directly read rather than memory map. 于小文件，直接读取而不是内存映射。
          Utils.tryWithSafeFinally { //异常不隐藏，不覆盖
            val buf = ByteBuffer.allocate(blockSize.toInt)
            // 用从通道读取的数据填充缓冲区。
            JavaUtils.readFully(channel, buf)
            // 翻转这个缓冲区。将限制设置为当前位置，然后将位置设置为0。如果标记被定义，那么它将被丢弃。
            buf.flip()
