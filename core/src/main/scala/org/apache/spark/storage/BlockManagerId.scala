 ...
 * modified from outside this class.
  *
  * 这个类的前两个构造函数都是私有的，以确保块managerid对象只能在伴生对象中使用apply方法创建。这允许删除ID对象。
  * 此外，构造函数参数是私有的，以确保不能从这个类的外部修改参数。
 */
@DeveloperApi
class BlockManagerId private (
  ...
  private def this() = this(null, null, 0, None)  // For deserialization only 仅供反序列化
