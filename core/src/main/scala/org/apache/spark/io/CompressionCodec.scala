 ...
 * to be used in block storage.
  * CompressionCodec允许定制选择不同的压缩实现来在块存储中使用。
 *
 ...
 * This is intended for use as an internal compression utility within a single Spark application.
  * 一个编解码器的连线协议不能保证在不同版本的Spark上兼容。这是在单个Spark应用程序中用作内部压缩实用程序的目的。
  *
  *   为了节省磁盘存储空间，有些情况下需要对Block进行压缩，根据配置属性spark.io.compression.codec来确定要使用的压缩算法（默认为snappy,
  * 此压缩算法在牺牲少量压缩比例的条件下，却极大地提高了压缩速度），并且生成SnappyCompressionCodec的实例。
 */
@DeveloperApi
