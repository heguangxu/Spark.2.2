 ...
 * Operations are not thread-safe.
  * 一个累加器的集合，表示关于读取shuffle数据的指标。操作不是线程安全的。
 */
@DeveloperApi
class ShuffleReadMetrics private[spark] () extends Serializable {
   ...
   * still not finished processing block A, it is not considered to be blocking on block B.
    *
    * 用于等待远程shuffle块的时间。这只包括在shuffle输入数据上阻塞的时间。例如，如果在任务仍未完成处理块A时，块B被提取，则不被认为阻塞B块。
    *
   */
   ...
   * [[TempShuffleReadMetrics]] into `this`.
    *
    * 重新设置当前度量的值(' this ')并将所有独立的[[TempShuffleReadMetrics]]合并到“this”中。
   */
 ...
 * last.
  *
  * 一个临时的shuffle读取指标holder，用于收集每一个拖拽依赖的指标，并最终将所有的临时指标合并到[[ShuffleReadMetrics]]中。
 */
