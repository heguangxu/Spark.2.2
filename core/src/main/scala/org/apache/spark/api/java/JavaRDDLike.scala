 ...
 * from the trait. See SPARK-3266 for additional details.
  *
  * 为解决https://issues.scala-lang.org/browse/si-8905，实现javarddlike应该扩展这个虚拟抽象类
  * 而不是直接继承这个特质。详情见spark-3266。
  *
 */
...
trait JavaRDDLike[T, This <: JavaRDDLike[T, This]] extends Serializable {
  ...
  /** A unique ID for this RDD (within its SparkContext).
    * RDD独一无二的ID值（在SparkContext中）
    * */
  def id: Int = rdd.id

  /** Get the RDD's current storage level, or StorageLevel.NONE if none is set.
    *   得到当前RDD的存储级别，如果没有返回none
    * */
   ...
   * subclasses of RDD.
    *
    * RDD内部的方法，如果计算这个RDD会从缓存中读取用不应该直接调用这个方法，但是可以实现这个方法。，
   */
   ...
   * Return a new RDD by applying a function to all elements of this RDD.
    * 返回一个新的RDD,这个老的RDD中的每个元素都执行指定的函数
   */
   ...
   * of the original partition.
    * 这个RDD每个分区执行一个函数，返回一个新的RDD,同时跟踪原分区索引。
   */
   ...
   *  RDD, and then flattening the results.
    *  首先应用一个函数返回该RDD的所有元素的一个新的RDD，然后压扁的结果。
   */
