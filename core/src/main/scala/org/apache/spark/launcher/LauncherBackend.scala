 ...
 * See `LauncherServer` for an explanation of how launcher communication works.
  *
  * 可以用来与启动服务器对话的类。用户应该扩展这个类来为抽象方法提供实现。
  *
  * 请参阅“LauncherServer”，以了解发射器通信是如何工作的。
 */
private[spark] abstract class LauncherBackend {
  ...
  /** local模式下，port和secret都为none，直接返回
    * local[*]模式下，port和secret都为none，直接返回
    * local[M,N]模式下，port和secret都为none，直接返回
    */
  def connect(): Unit = {
