...
private[spark] object ClosureCleaner extends Logging {
   ...
   * explicitly reference unserializable objects.
    * 更具体地说，这使得给定的闭包可序列化，只要它不显式引用不可序列化的对象。
    *
   ...
   *
    * 其机制是遍历封闭闭包的层次结构，并以实际中没有实际使用的方式将任何引用设置为空，但仍然包含在编译的匿名类中。
    * 注意，简单地将封闭的闭包进行更改是不安全的，因为其他代码路径可能依赖于它们。相反，我们克隆每个封闭闭包并相应地设置父指针。
    *
   ...
   * inner closure and record all the fields referenced in the process.
    *
    * 默认情况下，闭包是可传递的。这意味着我们检测是否包含了初始的对象，无论是直接的还是间接的，如果没有，
    * 则将这些闭包从层次结构中分离出来。换句话说，除了取消未使用的字段引用外，我们还将空出任何父指针，
    * 该指针指的是在开始关闭时不实际需要的封闭对象。我们通过跟踪通过内部闭包调用的所有方法树来确定传递性，
    * 并记录过程中引用的所有字段。
   *
  ...
  private def clean(
    ...
    // 递归获取所有闭包及最外部对象的class实例和对象实例。判断逻辑是func中包含$outer域，而且不为null,这里要说明的是，Scala会
    // 为每个函数合乘对象，每个函数都有一个$outer，但是只有函数是闭包时，$outer才不为空
    ...
    // 此方法主要使用asm框架访问class对象，这是一个典型的访问者模式的实现，此处朱勇主要检查闭包中是否有return语句，这在Spark中是不允许的。
    ...
    if (accessedFields.isEmpty) {
      ...
      // 如前面介绍的SomethingNotSerializable示例中，Scop("two")引用外部的Scop("one")的方法，而此方法中又引用Scope("one")
      // 的外部SomethingNotSerializabkle对象的域，因此需要递归查找每个对象实际被引用的域。
    ...
    // 校验func是否可被序列化，若不能被序列化，则排除异常快速失败
