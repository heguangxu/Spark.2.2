...
/** CallSite represents a place in user code. It can have a short and a long form.
  * CallSite表示用户代码中的一个位置。它可以有一个短而长的form。
  * */
...
private[spark] object Utils extends Logging {
   ...
   * the parameter transferToEnabled should be configured by spark.file.transferTo = [true|false].
    *
    * 将输入流中的所有数据复制到OutputStream。默认情况下，除非显式地设置transferToEnabled为true，
    * 否则默认禁用文件流的NIO方法。
    *
    * 启用的参数应该由spark.file配置。transferTo =(真|假)。
    *
    * 优化：设置transferToEnabled为真使用netty的NIO方式传输数据，会快很多
    *
   */
  def copyStream(
      in: InputStream,
   ...
   * the requested file.
      *
      *
      * 下载文件或目录到目标目录。支持以多种方式获取文件，包括基于URL参数的HTTP、hadoop兼容文件系统和标准文件系统上的文件。
      * 抓取目录只支持hadoop兼容的文件系统。
      *
      * 如果“useCache”是正确的，那么首先尝试将该文件获取到一个本地缓存，该缓存由运行相同应用程序的执行器共享。
      * “useCache”主要用于执行器，而不是本地模式。
      *
      * 如果目标文件已经存在，并且有不同于所请求文件的内容，则抛出SparkException。
   */
  def fetchFile(
      url: String,
      targetDir: File,
      ...
      timestamp: Long,
      useCache: Boolean) {
    // 解码文件的Url,组成file:///模式，作为本地下载文件的全名称
    val fileName = decodeFileNameInURI(new URI(url))
    val targetFile = new File(targetDir, fileName)
    // 优化：这里有一个优化点，可以缓存用户的jar包，这样速度会快些，但是可能造成jar包不更新
    val fetchCacheEnabled = conf.getBoolean("spark.files.useFetchCache", defaultValue = true)
    if (useCache && fetchCacheEnabled) {
      // 使用缓存jar包，先重新命名，后拷贝文件
      ...
      // 只有一个executor实体。FileLock只用于控制执行器下载文件的同步，无论锁类型(强制或警告)，它都是安全的。
    ...
    if (fileName.endsWith(".tar.gz") || fileName.endsWith(".tgz")) {
      ...
      // 执行一个命令并得到它的输出，如果它生成的代码不是0，则抛出异常。
    ...
    // 默认情况下，Windows不允许对非管理员用户进行读取权限，并显式地向所有者添加读权限。
    if (isWindows) {
  ...
  /**
    * 比较两个文件是否相同
    * 1.如果两个文件都是目录：比较文件数量，和每个文件是否相同
    * 2.如果两个文件都是文件：比较每个文件是否相同
    * @param file1
    * @param file2
    * @return
    */
  ...
  private def doFetchFile(
    ...
    val targetFile = new File(targetDir, filename)
    val uri = new URI(url)
    // 优化：是否缓存jar包，当依赖jar文件大的时候，使用比较好，但是注意这样可能导致jar包不更新
    ...
    Option(uri.getScheme).getOrElse("file") match {
      ...
      case "http" | "https" | "ftp" =>
        ...
        // 优化：获取远程文件（jar包）的超时时间，如果远程文件特别大，网络不好的的时候，建议调大些
        val timeoutMs =
   ...
   * hostname it reports to the master.
    * 允许设置自定义主机名，因为当我们在Mesos上运行时，我们需要使用它向主服务器报告的相同主机名。
   */
  ...
  def executeCommand(
    ...
    // 使用指定的操作系统程序和参数构造流程构建器。这是一个方便的构造函数，它将进程构建器的命令设置为一个字符串列表，
    // 其中包含与{@code命令}数组相同的字符串，顺序相同。不检查{@code命令}是否对应于一个有效的操作系统命令。
   ...
   * spark-started JVM process .
    *
    * 把一个代码块当做执行的单元，如果有任何未捕获的异常，那么就停止SparkContext
    *
    * 注意:此方法由驱动端组件调用，以避免用户端停止启动的JVM进程;相反，在spark启动的JVM进程中调用tryOrExit。
    *
    */
  ...
  /** Executes the given block. Log non-fatal errors if any, and only throw fatal errors
    * 执行给定的代码块。如果有错误，记录非致命错误，并且只抛出致命错误
    * */
   ...
   * exception from the original `out.write` call.
    *
    * 执行一个代码块，然后最后一个块，但是如果最终块中发生异常，不要抑制原始的异常。
    *
    *  这主要是一个“finally { out.close() }”块的问题，在这里需要调用close来清理“out”，
    *  但是如果在‘out.write’中发生了异常情况。它很可能被破坏了。关闭也会失败。这样就会抑制原来的可能更有意义的异常
   */
   ...
   * @param skipClass Function that is used to exclude non-user-code classes.
    ...
    *   最长栈：org.apache.spark.api.java.JavaSparkContext<init>(JavaSparkContext.scala:61) org.apache.spark.
    *          examples.JavaWordCount,main(JavaWordCount.java:44)
    *
    ...
    *
    *     val conf = new SparkConf().setMaster("local").setAppName("lcc_map");
    ...
    *       println("ste="+ste);
    *       println("ste.getMethodName="+ste.getMethodName);
    *       println("te.getMethodName.containse.="+ste.getMethodName.contains("getStackTrace"));
    *       println("ste.getClassName="+ste.getClassName);
    *       println("ste.getLineNumber="+ste.getLineNumber);
    *       println("ste.getFileName="+ste.getFileName);
    ...
    *             println("callStack(0)="+callStack(0));
    *             null
    *           } else {
    ...
    * te.getMethodName.containse.=true
    * ste.getClassName=java.lang.Thread
    * ste.getLineNumber=-1
    * ste.getFileName=null
    *
    * ste=scalaTest.Map$.main(Map.scala:21)
    * ste.getMethodName=main
    * te.getMethodName.containse.=false
    * ste.getClassName=scalaTest.Map$
    * ste.getLineNumber=21
    * ste.getFileName=Map.scala
    *
    * callStack=ArrayBuffer(<unknown>, scalaTest.Map$.main(Map.scala:21))
  ...
  def getCallSite(skipClass: String => Boolean = sparkInternalExclusionFunction): CallSite = {
    ...
    //  Thread.currentThread 意思 Returns a reference to the currently executing thread object.
    // 返回当前正在执行的线程对象的引用。
    Thread.currentThread.getStackTrace().foreach { ste: StackTraceElement =>
  ...
  */
   ...
   * overridden by the `SPARK_USER` environment variable.
    *
    * 返回当前用户名。这是当前登录的用户，除非它被“SPARK_USER”环境变量覆盖。
   */
  ...
  def getDynamicAllocationInitialExecutors(conf: SparkConf): Int = {
    ...
    // 如果初始化给的executor 小于 动态分配范围的最小值 那么记录一个warn
    ...
    // 如果executor实例的大小 小于   动态分配范围的最小值 那么记录一个warn
    ...
    // 也就是说 动态分配范围的最小值   《  初始化给的executor
    //          动态分配范围的最小值  《   executor实例的大小

    // Seq是序列，元素有插入的先后顺序，可以有重复的元素。是一个特质，list是它的一个实现类，
    // 返回的是一个list。scala> Seq(1, 1, 2) res3: Seq[Int] = List(1, 1, 2)
    // 这里是得到  (分配的最小executor数目，初始化时候的executor数目，executor实例的大小（没有默认为0） )
    // 这三个中的最大值
   ...
   * only the "spark.jars" property.
    *
    * 在YARN模式中，此方法返回"spark.jars"和"spark.yarn.dist.jars" 属性指向的一个独特的jar文件，而在其他模式中，
    * 它返回仅由“"spark.jars"指向的jar文件。
   */
 ...
 * very helpful to track which upper level job issues it.
  *
  * 用于在HDFS和Yarn上设置Spark调用上下文的实用程序类。“context”将由传入的参数构建。
  *
  * 当Spark应用程序在Yarn和HDFS上运行时，它的调用上下文将被写入到Yarn RM审计日志和hdfs-audit.log中。这可以帮助用户
  * 更好地诊断和理解特定应用程序如何影响Hadoop系统的部分和可能产生的问题(例如overloading NN)。在HDFS-9184中提到的HDFS，
  * 对于一个给定的HDFS操作来说，它非常有助于跟踪哪个上层的作业问题。
  *
 *
