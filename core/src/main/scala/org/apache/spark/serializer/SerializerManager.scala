 ...
 * components, including automatic selection of which [[Serializer]] to use for shuffles.
  *
  * 为各种Spark组件配置序列化、压缩和加密的组件，包括自动选择用于洗牌的[[Serializer]]。
 */
private[spark] class SerializerManager(
   ...
   * loaded yet.
   *
   * 使用压缩编解码器。请注意，“lazy”val是必要的，因为我们希望将压缩编解码器的初始化延迟到第一次使用。
   * 原因是火花程序可以在第三方jar使用用户定义的编解码器,这在Executor.updateDependencies加载。
   * 当BlockManager初始化时，用户级jar还没有加载。
   *
   *
   * */
  ...
  /** Serializes into a stream.
    * 数据流序列化方法
    * 如果写入存储体系的数据本身是序列化的，那么读取时应该对其反序列化。dataSerializeStream方法使用compressionCodec对文件输入流进行压缩
    * 和序列化处理。
    * */
