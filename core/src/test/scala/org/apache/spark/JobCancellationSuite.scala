...
class JobCancellationSuite extends SparkFunSuite with Matchers with BeforeAndAfter
  ...
  test("cluster mode, fair scheduler") {
    val conf = new SparkConf().set("spark.scheduler.mode", "FAIR")
    // F:\002-spark\01-sourceCode\spark-2.2.0\core\src\test\resources\fairscheduler-with-invalid-data.xml 查看这个文件有详细说明
    val xmlPath = getClass.getClassLoader.getResource("fairscheduler.xml").getFile()
    // 设置Spark资源调度池：conf.set("spark.scheduler.allocation.file", fairscheduler);
    conf.set("spark.scheduler.allocation.file", xmlPath)
    sc = new SparkContext("local-cluster[2,1,1024]", "test", conf)
    testCount()
    testTake()
