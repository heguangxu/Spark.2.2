...
class KryoSerializerAutoResetDisabledSuite extends SparkFunSuite with SharedSparkContext {
  ...
  /**
    * spark.shuffle.sort.bypassMergeThreshold:传递到reduce端再做合并（merge）操作的阈值，如果partition的数量
    * 小于spark.shuffle.sort.bypassMergeThreshold的值，则不需要在Executor执行聚合和排序操作，只需要将各个partition
    * 直接写到Exector的存储文件，最后在reduce端再做串联。通过配置spark.shuffle.sort.bypassMergeThreshold可以修改
    * spark.shuffle.sort.bypassMergeThreshold的大小，在分区数量小的时候提升计算引擎的性能。bypassMergeThreshold
    * 的默认值是200；
    *
    * bypassMergeSort:这个属性没找到。这个是标记是否传递到reduce端再做合并和排序，既是否直接将各个partition直接写到Executor
    * 的存储文件，当没有定义aggregator,ordering函数，并且partition的数量小于等于bypassMergeThreshold时，bypassMergeSort
    * 为true.如果bypassMergeSort为true，map中间结果将直接输出到磁盘，此时不会占用多大内存，避免了内存撑爆的问题。
    *
    */
