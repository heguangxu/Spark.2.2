...
private[sql] object Dataset {
  ...
  /**
    ...
    * 是sql执行的关键类，返回一个dataframe类型的对象。QueryExecution类中的成员都是lazy的，
    * 被调用时才会执行。只有等到程序中出现action算子时，才会调用 queryExecution类中的executedPlan成员，
    * 原先生成的逻辑执行计划才会被优化器优化，并转换成物理执行计划真正的被系统调用执行。
    * @param sparkSession
    * @param logicalPlan
    * @return
    */
...
class Dataset[T] private[sql](
  ...
  /**
   * Compose the string representing rows for output 组合表示输出行的字符串。
   *
   * @param _numRows Number of rows to show
   * @param truncate If set to more than 0, truncates strings to `truncate` characters and
   *                   all cells will be aligned right.
    *                   如果设置为大于0，将字符串截断为“truncate”字符，所有单元格将对齐。
   */
  private[sql] def showString(_numRows: Int, truncate: Int = 20): String = {
    // 传入的行数和0相比较，要大的值
    val numRows = _numRows.max(0)
    // toDF()是new Dataset，然后take()==>head(n)==》withAction()
   ...
   * and all cells will be aligned right. For example:
    * 以表格形式显示数据集。字符串超过20个字符将被截断，所有单元格将被对齐。例如:
   * {{{
   ...
   * @since 1.6.0
    *
    *         // truncate:是否截断长字符串。如果是真的，字符串超过20个字符将被截断，所有单元格将被对齐。
    *         默认是20个字符串
   */
   ...
   * @param truncate Whether truncate long strings. If true, strings more than 20 characters will
   *              be truncated and all cells will be aligned right
    *              是否截断长字符串。如果是真的，字符串超过20个字符将被截断，所有单元格将被对齐。
   *
   * @group action
   * @since 1.6.0
   */
  ...
  /**
   * Returns the first `n` rows. 返回前N条数据
   *
   ...
   * temporary table is tied to the [[SparkSession]] that was used to create this Dataset.
    * 将此数据集注册为使用给定名称的临时表。这个临时表的生命周期与用于创建该数据集的[[SparkSession]]绑定在一起。
   *
    * registerTempTable 这个方法被新方法替代了createOrReplaceTempView(viewName)
   * @group basic
   * @since 1.6.0
   */
   ...
   * user-registered callback functions.
    * 使用数据集操作来跟踪QueryExecution和时间成本，然后向用户注册的回调函数报告。
   */
