 ...
 * here for backward compatibility.
  *
  * 这个类是spark1.x构建的，我们保留这个类，为了兼容以前的，这个被SparkSession代替
 *
...
class SQLContext private[sql](val sparkSession: SparkSession)
   ...
   * other things.
    *
    * 返回一个SQLContext作为一个新的会话session，使用不同的Sql配置，临时表，注册的函数
    * 但是共同分享相同的SparkContext，缓存的data以及其他东西
   *
   * @since 1.6.0
   */
  ...
  // 禁用样式检查器，所以“implicits”对象可以从小写i开始。
  /**
   * :: Experimental ::
   * (Scala-specific) Implicit methods available in Scala for converting
   * common Scala objects into `DataFrame`s.
    * Scala中可用于将普通Scala对象转换为“DataFrame”的隐式方法。
   *
   * {{{
   ...
   * 从一个Product类型的RDD中创建一个DataFrame(例如，case类、元组)。
   * @group dataframes
   * @since 1.3.0
   */
  @Experimental
